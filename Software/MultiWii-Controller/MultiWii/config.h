...
/*************************************************************************************************/
  ...
  /**************************************************************************************/
    ...
    /***************************    independent sensors    ********************************/
      ...
      #define FORCE_ACC_ORIENTATION(Y, X, Z)  {imu.accADC[PITCH]  =  Y; imu.accADC[ROLL]  = -X; imu.accADC[YAW]  = -Z;}
      #define FORCE_GYRO_ORIENTATION(Y, X, Z) {imu.gyroADC[PITCH] = -Y; imu.gyroADC[ROLL] =  X; imu.gyroADC[YAW] = -Z;}
      #define FORCE_MAG_ORIENTATION(Y, X, Z)  {imu.magADC[PITCH]  =  X; imu.magADC[ROLL]  =  Y; imu.magADC[YAW]  = -Z;}
  ...
  /********************************  PID Controller *********************************/
    ...
    */
     ...
     * room for servo travel, then you must enable and set all three following options */
     //#define SERVO_MIN  {600, 600, 600, 600, 600, 600, 600, 600}  //I manually wrote 600 here (was 1020 before)!!
     //#define  SERVO_MAX {2400, 2400, 2400, 2400, 2400, 2400, 2400, 2400}  //I manually wrote 2400 here (was 1020 before)!!
 ...
 */
  ...
  /********                          Failsafe settings                 ********************/
    ...
    #define FAILSAFE_DELAY     4                      // Guard time for failsafe activation after signal lost. 1 step = 0.1sec - 1sec in example
    #define FAILSAFE_OFF_DELAY 200                    // Time for Landing before motors stop in 0.1sec. 1 step = 0.1sec - 20sec in example
    #define FAILSAFE_THROTTLE  1620                   // (*) Throttle level used for landing - may be relative to MINTHROTTLE - as in this case
  ...
  /**************************************************************************************/
   ...
   /* GPS protocol 
    ...
    // If your I2C GPS board has support enabled
