...
/**************************************************************************************/
    ...
    #else
      volatile uint16_t atomicServo[8] = {8000,8000,8000,8000,8000,8000,8000,8000}; //original line
      //volatile int16_t atomicServo[8] = {8000,8000,8000,8000,8000,8000,8000,8000};  // modified line by me
    #endif
  #endif
#endif
  ...
  // don't add offset for camtrig servo (SERVO3)
    ...
    for(i=SERVO_START-1; i<SERVO_END; i++) {
      if(i < 2) {
        ...
        //servo[i] = map(servo[i], 1020,2000, 600, 2400);
        }
    #if defined(HELICOPTER) && (YAWMOTOR)
      if(i != 5) // not limit YawMotor
    #endif
        // I commented this line out, so the servos are not limited in range any more (hopefully :) ) 
        servo[i] = constrain(servo[i], 1005, 2500); // limit the values (my own values)
        
        //servo[i] = constrain(servo[i], conf.servoConf[i].min, conf.servoConf[i].max); // limit the values (original)
    }
